    ...
    async def get_server_and_tool_info(
        ...
        async with MCPClient(self.server_registery) as client:
            for server_name in enabled_servers:
                ...
                # Retry mechanism for individual servers
                ...
                for server_attempt in range(max_server_retries):
                    try:
                        ...
                        logger.error(
                            f"MC: Failed to get info for server '{server_name}' (attempt {server_attempt + 1}/{max_server_retries}): {e}"
                        )
                        if server_attempt < max_server_retries - 1:
                            logger.info(f"MC: Retrying server '{server_name}' in 1 second...")
                            await asyncio.sleep(1)
                        else:
                            # Final failure for this server
                            if server_name not in servers_info:
                                servers_info[server_name] = {}
                        continue  # Continue retry loop
                    except Exception as e:
                        logger.error(
                            f"MC: Unexpected error for server '{server_name}' (attempt {server_attempt + 1}/{max_server_retries}): {e}"
                        )
                        if server_attempt < max_server_retries - 1:
                            await asyncio.sleep(1)
                        else:
