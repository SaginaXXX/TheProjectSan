...
# 面向流式输出（async iterator）的文本处理器。

# 功能一：实时句子切分（优先用 pysbd，不支持的语言回退正则）。

# 功能二：首句加速（允许把第一句遇到逗号就先切出来，提升“首响时间”）。

# 功能三：标签解析（可配置允许的标签，比如 <think>），还能处理嵌套标签，并在产出的句子里带上“当前生效的标签上下文”。

# 功能四：混流支持：输入既可能是字符串分片，也可能是字典（例如中途插入控制消息）；字典会原样透传，但在透传前先把当前能成句的内容吐出去，保证时序正确。

# 功能五：完整响应拼接：一边切句一边累计形成 complete_response。
