...
# 从文件名中提取机器编号
def extract_machine_id_from_filename(filename: str) -> Optional[str]:
    """
    从文件名中提取机器编号
    支持多种命名格式：machine_1.mp4, 洗衣机_A.mp4, 1号.mp4 等
    """
    # 移除文件扩展名
    ...
    # 定义匹配模式
    patterns = [
        r'machine[_\-\s]*([a-z0-9]+)',  # machine_1, machine-A, machine 1
        r'洗衣机[_\-\s]*([a-z0-9]+)',   # 洗衣机_1, 洗衣机-A
        r'washing[_\-\s]*machine[_\-\s]*([a-z0-9]+)',  # washing_machine_1
        r'^([a-z0-9]+)号?$',            # 1号, A号, 或直接 1, A
        r'^([a-z0-9]+)[_\-\s]*machine',  # 1_machine, A-machine
    ]
    
    for pattern in patterns:
        match = re.search(pattern, name_without_ext)
        if match:
            return match.group(1).upper()  # 统一转换为大写
...
def init_webtool_routes(default_context_cache: ServiceContext) -> APIRouter:
    ...
    async def get_laundry_video_list():
        """获取洗衣机教程视频列表"""
        try:
            ...
            for video_file in videos_dir.iterdir():
                if video_file.is_file() and video_file.suffix.lower() in supported_formats:
                    ...
                    # 尝试从文件名提取机器编号
                    machine_id = extract_machine_id_from_filename(video_file.name)
                    
                    videos.append({
                        "id": f"video_{len(videos):03d}",
                        "name": video_file.stem,
                        "filename": video_file.name,
                        "machine_id": machine_id,
                        "path": str(video_file),
                        "url_path": f"/videos/{video_file.name}",
                        "size_bytes": file_size,
                        "size_mb": round(file_size / (1024 * 1024), 2),
                        "format": video_file.suffix.lower(),
                        "category": "laundry_tutorial"
                    })
            ...
            logger.error(f"Error getting laundry video list: {e}")
            return Response(
                content=json.dumps({"error": f"Failed to get laundry video list: {str(e)}"}),
                status_code=500,
                media_type="application/json",
            )

    @router.post("/api/videos/upload")
    async def upload_laundry_video(file: UploadFile = File(...)):
        """上传洗衣机教程视频文件"""
        try:
            # 验证文件类型
            allowed_extensions = {'.mp4', '.avi', '.mov', '.mkv', '.webm'}
            ...
            # 验证文件大小（限制为1GB）
            max_size = 1024 * 1024 * 1024  # 1GB (教程视频可能比广告大)
            contents = await file.read()
            if len(contents) > max_size:
                return Response(
                    content=json.dumps({
                        "error": f"文件太大。最大允许大小: 1GB，当前文件: {len(contents)/(1024*1024*1024):.1f}GB"
                    }),
                    status_code=400,
                    media_type="application/json",
                )
            ...
            # 获取文件信息
            file_size = len(contents)
            machine_id = extract_machine_id_from_filename(filename)
            
            logger.info(f"Successfully uploaded laundry video: {filename} ({file_size/(1024*1024):.2f}MB)")
            
            return {
                "status": "success",
                "message": f"洗衣机教程视频 '{filename}' 上传成功",
                "file_info": {
                    "name": original_name,
                    "filename": filename,
                    "machine_id": machine_id,
                    "path": str(file_path),
                    "url_path": f"/videos/{filename}",
                    "size_bytes": file_size,
                    "size_mb": round(file_size / (1024 * 1024), 2),
                    "format": file_extension,
                    "category": "laundry_tutorial"
            ...
            logger.error(f"Error uploading laundry video: {e}")
            return Response(
                content=json.dumps({"error": f"上传失败: {str(e)}"}),
                status_code=500,
                media_type="application/json",
    ...
    async def delete_laundry_video(filename: str):
        """删除洗衣机教程视频文件"""
        try:
            videos_dir = Path("videos")
            file_path = videos_dir / filename
            ...
            logger.info(f"Successfully deleted laundry video: {filename}")
            
            return {
                "status": "success",
                "message": f"洗衣机教程视频 '{filename}' 删除成功",
                "deleted_file": filename
            }
            
        except Exception as e:
            logger.error(f"Error deleting laundry video: {e}")
            return Response(
                content=json.dumps({"error": f"删除失败: {str(e)}"}),
                status_code=500,
                media_type="application/json",
