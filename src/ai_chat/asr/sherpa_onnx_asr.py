    ...
    def _create_recognizer(self):
        if self.model_type == "transducer":
            ...
            if not self.sense_voice or not os.path.isfile(self.sense_voice):
                # Avoid heavy auto-downloads on first boot. Fail fast and let caller decide.
                logger.error(
                    "SenseVoice model not found locally. Skipping ASR initialization (no auto-download)."
                )
                raise FileNotFoundError(
                    "SenseVoice model missing. Provide local paths in conf.yaml to enable ASR."
                )
