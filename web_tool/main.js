...
startRecordingBtn.addEventListener('click', async () => {
    try {
        ...
        // Attach streaming handler for 1s chunks
        recorder.onChunk = async (blob) => {
            try {
                // Convert chunk to proper 16k/mono WAV
                ...
                const formData = new FormData();
                formData.append('file', wavBlob, `chunk_${Date.now()}.wav`);

                const resp = await fetch(`${API_BASE_URL}/api/asr-chunk`, {
                    method: 'POST',
                    body: formData,
                });
                if (!resp.ok) return;
                ...
                if (data && typeof data.text === 'string' && data.text.length) {
                    ...
                    const prev = transcriptionArea.value.trim();
                    transcriptionArea.value = prev ? `${prev} ${data.text}` : data.text;
                }
                tmpCtx.close();
            } catch (err) {
                // Swallow per-chunk errors to keep stream flowing
                console.warn('ASR chunk error:', err);
            }
        };
...
stopRecordingBtn.addEventListener('click', async () => {
    try {
        ...
        asrStatus.textContent = 'Stopped. Using streamed partial transcription above.';
        asrStatus.className = 'status success';
    } catch (error) {
        asrStatus.textContent = 'Error: ' + error.message;
        asrStatus.className = 'status error';
